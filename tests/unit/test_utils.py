...
class TestGetPyprojectPath:
    ...
    def test_in_parent_directory(
        ...
        mock_path.cwd.return_value = sub_dir
    ...
    @patch("autoimport.utils.Path")
    def test_not_found(self, mock_path: MagicMock) -> None:
        """
        Given: no `pyproject.toml` in the `cwd` or parent dirs,
        When: the `get_pyproject_path` function is invoked,
        Then: `None` is returned
        """
        mock_path.cwd.return_value = Path("/nowhere")
